# 评价与信用积分系统实现文档

## 功能概述

本系统实现了完整的评价与信用积分系统，包括以下核心功能：

### 1. 评价系统
- 买家完成订单后可以对商品和卖家进行评价
- 支持1-5星评分（商品评分和卖家评分分开）
- 支持文字评价和图片上传（支持多张图片）
- 支持匿名评价选项
- 商品详情页显示所有评价，支持查看评价图片
- 每个订单只能评价一次，防止重复评价
- 评价后自动更新卖家信用积分
- **修复说明**：已修复创建评价时 `updated_at` 字段为空导致的数据库错误，现在创建评价时会自动设置 `createdAt` 和 `updatedAt` 时间戳

### 2. 信用积分系统
- 信用分计算规则：基础分100 + 完成交易数×2 + 平均评分×20 - 差评数×10
- 五个信用等级：
  - 等级1：新手（总分<150 或 交易数<5）🌱
  - 等级2：铜牌卖家（总分≥150 且 交易数≥5）🥉
  - 等级3：银牌卖家（总分≥250 且 交易数≥20 且 平均评分≥3.5）🥈
  - 等级4：金牌卖家（总分≥400 且 交易数≥50 且 平均评分≥4.0）🥇
  - 等级5：钻石卖家（总分≥600 且 交易数≥100 且 平均评分≥4.5）💎
- 自动统计：已售商品数、已购商品数、好评率、评价详情等

## 数据库表结构

### review表（评价表）
```sql
CREATE TABLE `review` (
    `id` BIGINT NOT NULL AUTO_INCREMENT COMMENT '评价ID',
    `order_id` INT NOT NULL COMMENT '订单ID',
    `product_id` BIGINT NOT NULL COMMENT '商品ID',
    `buyer_id` INT NOT NULL COMMENT '买家ID',
    `seller_id` INT NOT NULL COMMENT '卖家ID',
    `product_rating` INT NOT NULL COMMENT '商品评分 1-5',
    `seller_rating` INT NOT NULL COMMENT '卖家评分 1-5',
    `comment` TEXT NULL COMMENT '评价内容',
    `review_images` TEXT NULL COMMENT '评价图片，多个用逗号分隔',
    `is_anonymous` BOOLEAN NOT NULL DEFAULT FALSE COMMENT '是否匿名评价',
    `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (`id`),
    UNIQUE INDEX `uk_order_id` (`order_id`)
);
```

### credit_score表（信用积分表）
```sql
CREATE TABLE `credit_score` (
    `id` BIGINT NOT NULL AUTO_INCREMENT COMMENT '信用积分ID',
    `user_id` BIGINT NOT NULL COMMENT '用户ID',
    `total_score` INT NOT NULL DEFAULT 100 COMMENT '总信用分',
    `level` INT NOT NULL DEFAULT 1 COMMENT '信用等级 1-5',
    `total_sales` INT NOT NULL DEFAULT 0 COMMENT '总销售数',
    `total_purchases` INT NOT NULL DEFAULT 0 COMMENT '总购买数',
    `average_seller_rating` DOUBLE NOT NULL DEFAULT 0.0 COMMENT '平均卖家评分',
    `positive_reviews` INT NOT NULL DEFAULT 0 COMMENT '好评数',
    `neutral_reviews` INT NOT NULL DEFAULT 0 COMMENT '中评数',
    `negative_reviews` INT NOT NULL DEFAULT 0 COMMENT '差评数',
    `updated_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (`id`),
    UNIQUE INDEX `uk_user_id` (`user_id`)
);
```

## 后端API接口

### 评价相关接口

#### 1. 创建评价
```
POST /api/reviews
Authorization: Bearer {token}

Request Body:
{
  "orderId": 1,
  "productRating": 5,      // 商品评分 1-5
  "sellerRating": 5,       // 卖家评分 1-5
  "comment": "商品很好",   // 评价内容（可选）
  "reviewImages": [],      // 评价图片URL数组（可选）
  "isAnonymous": false     // 是否匿名（可选）
}

Response:
{
  "success": true,
  "message": "评价成功",
  "data": { ... }
}
```

#### 2. 检查订单是否已评价
```
GET /api/reviews/check/{orderId}

Response:
{
  "success": true,
  "hasReviewed": false
}
```

#### 3. 获取商品的所有评价
```
GET /api/reviews/product/{productId}

Response:
{
  "success": true,
  "reviews": [ ... ],
  "averageRating": 4.5,
  "totalReviews": 10
}
```

#### 4. 获取卖家的所有评价
```
GET /api/reviews/seller/{sellerId}

Response:
{
  "success": true,
  "reviews": [ ... ]
}
```

### 信用积分相关接口

#### 1. 获取用户信用分
```
GET /api/credit-score/{userId}

Response:
{
  "success": true,
  "data": {
    "userId": 1,
    "username": "user123",
    "displayName": "用户名",
    "avatarUrl": "...",
    "totalScore": 150,
    "level": 2,
    "levelName": "铜牌卖家",
    "totalSales": 5,
    "totalPurchases": 3,
    "averageSellerRating": 4.5,
    "positiveReviews": 4,
    "neutralReviews": 1,
    "negativeReviews": 0,
    "totalReviews": 5,
    "positiveRate": 80.0
  }
}
```

#### 2. 更新信用分（管理员或系统调用）
```
POST /api/credit-score/update/{userId}
Authorization: Bearer {token}

Response:
{
  "success": true,
  "message": "信用分更新成功",
  "data": { ... }
}
```

## 前端组件

### 1. ReviewView.vue
评价页面，用户在确认收货后可以对订单进行评价。

功能：
- 商品评分（1-5星）
- 卖家评分（1-5星）
- 文字评价
- 匿名评价选项
- 提交评价

路由：`/review/:orderId`

### 2. ProductReviews.vue
商品评价展示组件，显示在商品详情页。

功能：
- 显示平均评分
- 显示所有评价列表
- 显示买家信息（或匿名）
- 显示评价时间、评分、评价内容

### 3. CreditScoreCard.vue
信用分展示卡片组件，显示在商品详情页的卖家信息区域。

功能：
- 显示卖家信用等级和图标
- 显示信用总分
- 显示已售/已购商品数
- 显示好评率
- 显示评价详情（好评/中评/差评分布）
- 显示平均卖家评分

## 使用流程

### 评价流程
1. 买家购买商品并支付
2. 卖家发货
3. 买家确认收货（订单状态变为DELIVERED）
4. 在"订单中心"显示"⭐ 评价订单"按钮
5. 点击按钮跳转到评价页面
6. 填写评分和评价内容
7. 提交评价
8. 系统自动更新卖家信用分
9. 按钮变为"✅ 已评价"（不可再次评价）

### 信用分更新时机
1. 买家确认收货时（自动更新买家和卖家的信用分）
2. 买家提交评价后（自动更新卖家的信用分）
3. 管理员手动触发更新

### 商品详情页展示
1. 卖家信用分卡片（显示卖家的信用等级、评分等）
2. 商品评价列表（显示该商品的所有评价）

## 核心功能实现

### 后端核心类

1. **Entity类**
   - `Review.java` - 评价实体
   - `CreditScore.java` - 信用积分实体

2. **Mapper接口**
   - `ReviewMapper.java` - 评价数据访问
   - `CreditScoreMapper.java` - 信用积分数据访问

3. **Service类**
   - `ReviewService.java` / `ReviewServiceImpl.java` - 评价业务逻辑
   - `CreditScoreService.java` / `CreditScoreServiceImpl.java` - 信用积分业务逻辑

4. **Controller类**
   - `ReviewController.java` - 评价API接口
   - `CreditScoreController.java` - 信用积分API接口

5. **DTO类**
   - `ReviewRequest.java` - 评价请求
   - `ReviewResponse.java` - 评价响应
   - `CreditScoreResponse.java` - 信用积分响应

### 前端核心文件

1. **Views**
   - `ReviewView.vue` - 评价页面

2. **Components**
   - `ProductReviews.vue` - 商品评价展示组件
   - `CreditScoreCard.vue` - 信用分卡片组件

3. **Router**
   - 添加了 `/review/:orderId` 路由

4. **更新的文件**
   - `ProductDetail.vue` - 添加了卖家信用分和商品评价显示
   - `OrderHistory.vue` - 添加了评价按钮和相关逻辑

## 注意事项

1. **评价规则**
   - 只有状态为DELIVERED的订单才能评价
   - 每个订单只能评价一次
   - 评分必须在1-5之间
   - 好评：4-5星，中评：3星，差评：1-2星

2. **信用分计算**
   - 新用户初始信用分为100分
   - 完成交易会增加信用分
   - 差评会降低信用分
   - 信用等级根据总分、交易数、平均评分综合计算

3. **数据一致性**
   - 订单确认收货时自动更新信用分
   - 提交评价时自动更新信用分
   - 使用事务保证数据一致性

## 测试建议

1. 测试评价功能
   - 创建订单 -> 支付 -> 确认收货 -> 评价
   - 测试重复评价（应该被阻止）
   - 测试匿名评价
   - 测试不同评分对信用分的影响

2. 测试信用分系统
   - 查看新用户的初始信用分
   - 完成多笔交易后查看信用分变化
   - 测试不同等级的晋升条件
   - 测试差评对信用分的影响

3. 测试前端展示
   - 商品详情页是否正确显示卖家信用分
   - 商品详情页是否正确显示评价列表
   - 订单中心是否正确显示评价按钮
   - 评价页面功能是否完整

## 已完成的文件

### 后端文件（Java）
✅ `/src/main/java/lut/cn/c2cplatform/entity/Review.java`
✅ `/src/main/java/lut/cn/c2cplatform/entity/CreditScore.java`
✅ `/src/main/java/lut/cn/c2cplatform/mapper/ReviewMapper.java`
✅ `/src/main/java/lut/cn/c2cplatform/mapper/CreditScoreMapper.java`
✅ `/src/main/java/lut/cn/c2cplatform/mapper/OrderMapper.java` (已修复)
✅ `/src/main/java/lut/cn/c2cplatform/dto/ReviewRequest.java`
✅ `/src/main/java/lut/cn/c2cplatform/dto/ReviewResponse.java`
✅ `/src/main/java/lut/cn/c2cplatform/dto/CreditScoreResponse.java`
✅ `/src/main/java/lut/cn/c2cplatform/service/ReviewService.java`
✅ `/src/main/java/lut/cn/c2cplatform/service/CreditScoreService.java`
✅ `/src/main/java/lut/cn/c2cplatform/service/impl/ReviewServiceImpl.java`
✅ `/src/main/java/lut/cn/c2cplatform/service/impl/CreditScoreServiceImpl.java`
✅ `/src/main/java/lut/cn/c2cplatform/service/impl/OrderServiceImpl.java` (已更新)
✅ `/src/main/java/lut/cn/c2cplatform/controller/ReviewController.java`
✅ `/src/main/java/lut/cn/c2cplatform/controller/CreditScoreController.java`

### 前端文件（Vue）
✅ `/frontend/src/views/ReviewView.vue`
✅ `/frontend/src/components/ProductReviews.vue`
✅ `/frontend/src/components/CreditScoreCard.vue`
✅ `/frontend/src/views/ProductDetail.vue` (已更新)
✅ `/frontend/src/views/OrderHistory.vue` (已更新)
✅ `/frontend/src/router/index.js` (已更新)

### 数据库文件
✅ `/init.sql` (已添加review和credit_score表)

## 总结

评价与信用积分系统已经完整实现，包括：
- ✅ 完整的评价功能（创建、查询、展示）
- ✅ 信用积分系统（自动计算、等级划分）
- ✅ 前端UI组件（评价页面、评价列表、信用分卡片）
- ✅ 数据库表结构
- ✅ 完整的API接口
- ✅ 自动更新机制

系统完全参照闲鱼的设计，提供了完善的用户信用体系和评价系统。

