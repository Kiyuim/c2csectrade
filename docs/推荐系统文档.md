# c2csectrade 智能推荐系统文档

## 系统概述

c2csectrade 实现了一个**企业级多层次混合推荐系统**，适用于二手商品交易平台的各类商品（数码、服装、图书、家居等）。系统结合了协同过滤、基于内容的推荐、神经网络、实时推荐和热度排序，为用户提供精准的个性化商品推荐。

## 架构设计

### 1. 三层推荐架构

#### 1.1 协同过滤 (Collaborative Filtering)
- **实现类**: `RecommendationEngineService`
- **算法**: Item-Based Collaborative Filtering
- **原理**: 通过用户浏览历史构建商品共现矩阵，使用余弦相似度计算商品间的相似度
- **权重**: 40%

**计算流程**:
```
1. 收集用户浏览历史 (Redis Sorted Set)
2. 构建商品共现矩阵
3. 计算余弦相似度: similarity = co-occurrence / sqrt(total1 * total2)
4. 存储Top 10相似商品到Redis (recommend:item-cf:{productId})
```

#### 1.2 基于内容的推荐 (Content-Based)
- **实现类**: `ContentBasedRecommendationService`
- **算法**: TF-IDF + 余弦相似度
- **特征**: 商品名称、描述、类别、价格区间、成色、位置
- **权重**: 40%

**特征提取**:
- **类别** (高权重): `cat:category`
- **商品名** (中文分词 + bigram): `name:word`
- **描述**: `desc:word`
- **价格区间**: `price:0-20`, `price:20-50`, `price:50-100`等
- **成色**: `condition:1-10`
- **位置**: `loc:location`

**相似度计算**:
```
1. TF-IDF向量化
   - TF = term_freq / total_terms
   - IDF = log(total_docs / doc_freq)
   - TF-IDF = TF * IDF

2. 余弦相似度
   - cosine_sim = dot_product / (norm1 * norm2)

3. 价格相似度 (指数衰减)
   - price_sim = exp(-relative_diff * 2)

4. 综合得分
   - final_score = 0.8 * cosine_sim + 0.2 * price_sim
```

#### 1.3 热度排序 (Popularity-Based)
- **实现类**: `HybridRecommendationService`
- **数据结构**: Redis Sorted Set
- **权重**: 20%

**行为权重**:
- 浏览 (view): 1.0
- 收藏 (favorite): 3.0
- 加购 (cart): 5.0
- 评价 (review): 8.0
- 购买 (order): 10.0

**热度衰减**: 每天自动衰减10%，防止老商品持续霸榜

### 2. 混合推荐策略

**实现类**: `HybridRecommendationService.getHybridSimilarProducts()`

```
综合得分 = 0.4 * CF得分 + 0.4 * 内容得分 + 0.2 * 热度得分
```

## 数据流设计

### 1. 用户行为追踪

#### 1.1 浏览历史
```java
// Redis Key: history:user:{userId}
// Data Structure: Sorted Set (score = timestamp)
historyService.recordView(userId, productId);
```

#### 1.2 用户兴趣画像
```java
// Redis Key: profile:user:{userId}
// Data Structure: Hash (tag -> interest_score)
// 自动从浏览历史提取标签并累加兴趣分
```

### 2. 推荐数据缓存

#### 2.1 协同过滤缓存
```
Key: recommend:item-cf:{productId}
Type: Sorted Set
Score: 相似度分数
Value: 相似商品ID
TTL: 永久 (每日更新)
```

#### 2.2 内容相似度缓存
```
Key: recommend:content:{productId}
Type: Sorted Set
Score: 相似度分数
Value: 相似商品ID
TTL: 7天
```

#### 2.3 热度缓存
```
Key: recommend:popularity
Type: Sorted Set
Score: 热度分数
Value: 商品ID
TTL: 永久 (实时更新 + 每日衰减)
```

## API接口

### 1. 猜你喜欢
```http
GET /api/recommendations/for-you?limit=10
```

**逻辑**:
1. 获取用户兴趣画像 (Top 3标签)
2. 根据兴趣标签搜索商品
3. 过滤已浏览和自己的商品
4. 返回个性化推荐

**未登录用户**: 返回热门商品

### 2. 相似商品推荐
```http
GET /api/recommendations/products/{id}/similar?limit=10
```

**逻辑**:
1. 使用混合推荐算法获取相似商品ID
2. 加载商品详情和媒体
3. 过滤不可用商品
4. 如果结果不足，补充同类别商品

### 3. 手动触发计算 (管理员)
```http
POST /api/recommendations/compute
Authorization: Admin
```

## 定时任务

### 每日凌晨3点自动计算
```java
@Scheduled(cron = "0 0 3 * * ?")
public void computeRecommendations()
```

**执行流程**:
1. 协同过滤计算 (5-10分钟)
2. 内容相似度计算 (10-20分钟)
3. 热度分数衰减 (1分钟)

## 性能优化

### 1. 离线计算
- 推荐计算在后台定时执行
- 不影响在线服务性能

### 2. Redis缓存
- 所有推荐结果预计算并缓存
- 查询时直接从Redis读取，延迟 < 10ms

### 3. 异步处理
- 用户行为追踪不阻塞主流程
- 使用消息队列异步处理

### 4. 批量操作
- 商品详情批量加载
- 减少数据库查询次数

## 前端集成

### 1. 主页推荐区块
```vue
<RecommendationSection
  title="🎯 猜你喜欢"
  subtitle="根据您的浏览历史推荐"
  :products="recommendedProducts"
/>
```

### 2. 商品详情页
```vue
<RecommendationSection
  title="👀 看了又看"
  subtitle="其他用户也浏览过这些商品"
  :products="similarProducts"
/>
```

### 3. 自动加载
```javascript
// 在商品详情页挂载时
onMounted(() => {
  trackProductView();      // 追踪浏览
  fetchSimilarProducts();  // 获取推荐
});
```

## 扩展性设计

### 1. 新增推荐算法
继承 `HybridRecommendationService` 并实现新的评分策略：
```java
public Map<Long, Double> getNewAlgorithmScores(Long productId) {
    // 实现新算法
}
```

### 2. 调整权重
修改 `HybridRecommendationService` 中的常量：
```java
private static final double CF_WEIGHT = 0.4;
private static final double CONTENT_WEIGHT = 0.4;
private static final double POPULARITY_WEIGHT = 0.2;
```

### 3. 添加新特征
在 `ContentBasedRecommendationService.extractTerms()` 中添加：
```java
// 添加品牌特征
if (product.getBrand() != null) {
    terms.add("brand:" + product.getBrand());
}
```

## 监控指标

### 1. 推荐覆盖率
- 有推荐结果的商品比例
- 目标: > 80%

### 2. 推荐准确率
- 点击率 (CTR)
- 转化率 (CVR)

### 3. 计算性能
- 协同过滤计算时间
- 内容相似度计算时间

### 4. 缓存命中率
- Redis缓存命中率
- 目标: > 95%

## 已知限制

1. **冷启动问题**: 新商品和新用户缺少历史数据
   - 解决方案: 使用基于内容的推荐和热门商品

2. **计算资源**: 大规模商品需要较长计算时间
   - 解决方案: 增量计算、分布式计算

3. **实时性**: 推荐结果每日更新一次
   - 解决方案: 引入实时流处理框架

## 高级功能实现

### 1. 深度学习模型 - 神经网络协同过滤 (NCF) ✅

**实现类**: `NCFRecommendationService`

**架构**:
- **Embedding层**: 用户和商品的向量表示 (32维)
- **GMF部分**: 广义矩阵分解 (元素级乘积)
- **MLP部分**: 多层感知器 (特征学习)
- **融合层**: NeuMF组合GMF和MLP

**核心方法**:
```java
// 预测用户-商品交互分数
double score = ncfService.predictScore(userId, productId);

// 在线学习：根据用户行为更新模型
ncfService.trainOnInteraction(userId, productId, "order");

// 获取NCF推荐
List<Long> recommendations = ncfService.getNCFRecommendations(
    userId, candidateProducts, 10
);
```

**优势**:
- 非线性特征学习
- 在线增量学习
- 个性化程度高

### 2. 实时推荐系统 ✅

**实现类**: `RealtimeRecommendationService`

**实时处理能力**:
- 用户会话跟踪（最近20次交互）
- 5分钟滑动时间窗口
- 趋势商品实时计算
- 即时个性化评分

**时间窗口聚合**:
```java
// 获取实时趋势商品
List<Long> trending = realtimeService.getTrendingProducts(30, 20);

// 获取实时个性化推荐
List<Long> recommendations = realtimeService.getRealtimeRecommendations(userId, 10);

// 处理用户交互
realtimeService.processInteraction(userId, productId, "view");
```

**特点**:
- 毫秒级响应
- 会话感知推荐
- 时间衰减机制
- 热点捕获

### 3. A/B测试框架 ✅

**实现类**: `ABTestService`

**支持的测试**:
- 多变体实验（2-10个变体）
- 流量分配控制
- 一致性哈希分配
- 统计显著性检验

**使用示例**:
```java
// 创建实验
Map<String, Double> variants = Map.of(
    "control", 0.2,      // CF算法
    "variant_a", 0.2,    // 基于内容
    "variant_b", 0.2,    // 混合推荐
    "variant_c", 0.2,    // NCF神经网络
    "variant_d", 0.2     // 实时推荐
);
abTestService.createExperiment("rec_algo_v1", "推荐算法对比", variants);

// 分配用户到变体
String variant = abTestService.assignUserToVariant(userId, "rec_algo_v1");

// 追踪指标
abTestService.trackClick(userId, experimentId, productId);
abTestService.trackConversion(userId, experimentId, productId, 99.99);

// 获取实验结果
Map<String, Map<String, Object>> results = abTestService.getExperimentResults("rec_algo_v1");
```

**指标追踪**:
- 曝光量 (Impressions)
- 点击率 (CTR)
- 转化率 (CVR)
- 收入 (Revenue)
- 显著性检验 (Chi-square)

### 4. 多目标优化 ✅

**实现类**: `MultiObjectiveOptimizationService`

**优化目标**:
1. **点击率 (CTR)** - 25%权重
2. **转化率 (CVR)** - 30%权重
3. **收入 (Revenue)** - 20%权重
4. **多样性 (Diversity)** - 15%权重
5. **新颖性 (Novelty)** - 10%权重

**核心算法**:
```java
// 多目标优化推荐
List<Long> optimized = multiObjectiveService.getMultiObjectiveRecommendations(
    userId, candidates, 10
);

// MMR多样性重排序
List<Long> diverse = multiObjectiveService.rerankForDiversity(
    recommendations, 0.7  // lambda参数
);

// Pareto前沿（非支配解）
List<ScoredCandidate> paretoFrontier = 
    multiObjectiveService.getParetoFrontier(candidates);
```

**算法实现**:
- **加权评分**: 线性组合多个目标
- **MMR重排序**: 平衡相关性和多样性
- **Pareto优化**: 找到非支配解集合

### 5. Redis深度集成 ✅

**实现类**: `RedisCacheService`

**高级功能**:
```java
// 分布式锁
String lockId = UUID.randomUUID().toString();
if (redisCacheService.acquireLock("product:123", lockId, 10)) {
    try {
        // 执行临界区代码
    } finally {
        redisCacheService.releaseLock("product:123", lockId);
    }
}

// 热点商品缓存
redisCacheService.updateHotProducts(productId, score);
List<Long> hotProducts = redisCacheService.getHotProducts(20);

// 批量操作（Pipeline）
Map<String, Object> batch = Map.of(
    "product:1", product1,
    "product:2", product2
);
redisCacheService.batchSet(batch, 60);

// 布隆过滤器（去重）
redisCacheService.bloomAdd("viewed:products", productId.toString());
boolean seen = redisCacheService.bloomContains("viewed:products", productId.toString());

// 限流控制
Long count = redisCacheService.incrementWithExpire("ratelimit:user:" + userId, 60);
if (count > 100) {
    throw new RateLimitException("请求过于频繁");
}
```

## 系统集成架构

```
用户请求
   ↓
A/B测试分流
   ↓
┌──────────────────────────────────────┐
│  推荐策略路由                          │
│  - Control: 协同过滤                   │
│  - Variant A: 基于内容                 │
│  - Variant B: 混合推荐                 │
│  - Variant C: NCF神经网络              │
│  - Variant D: 实时推荐                 │
└──────────────────────────────────────┘
   ↓
候选生成 (Candidate Generation)
   ↓
多目标优化 (Multi-Objective Optimization)
   ↓
MMR重排序 (Diversity Re-ranking)
   ↓
结果返回 + 指标追踪
```

## 性能优化策略

### Redis缓存分层
1. **L1缓存** - 热点数据（TTL: 5分钟）
2. **L2缓存** - 推荐结果（TTL: 1小时）
3. **L3缓存** - 模型参数（TTL: 1天）

### 异步处理
- 用户行为追踪 → RabbitMQ → 异步更新模型
- 推荐计算 → 后台定时任务
- 指标收集 → 消息队列 → 批量入库

### 分布式计算
- 协同过滤：离线批处理
- NCF训练：在线增量学习
- 实时推荐：流式处理

## 监控指标

### 业务指标
- **CTR**: 点击率 = 点击数 / 曝光数
- **CVR**: 转化率 = 订单数 / 点击数
- **GMV**: 成交总额
- **用户满意度**: 评分、停留时间

### 技术指标
- **响应时间**: P50 < 50ms, P99 < 200ms
- **缓存命中率**: > 95%
- **推荐覆盖率**: > 85%
- **多样性指标**: ILD (Intra-List Diversity)

### A/B测试指标
- **相对提升**: (变体指标 - 对照组指标) / 对照组指标
- **统计显著性**: p-value < 0.05
- **样本量**: 每组 > 1000

## API文档

### 1. 获取个性化推荐
```http
GET /api/recommendations/for-you?limit=10
Authorization: Bearer <token>

Response:
{
  "products": [...],
  "strategy": "ncf_neural",
  "experiment": "rec_algo_v1"
}
```

### 2. 获取相似商品
```http
GET /api/recommendations/products/123/similar?limit=10

Response:
{
  "products": [...],
  "algorithm": "hybrid",
  "diversity_score": 0.82
}
```

### 3. 获取实时趋势
```http
GET /api/recommendations/trending?minutes=30&limit=20

Response:
{
  "products": [...],
  "window": "30分钟",
  "updated_at": "2025-01-15T10:30:00Z"
}
```

## 搜索与高亮技术

本平台集成了 Elasticsearch 全文搜索引擎，并实现了关键词高亮显示功能，提升用户搜索体验。

### 1. 技术实现
- **搜索引擎**: Elasticsearch 8.11
- **集成框架**: Spring Data Elasticsearch
- **高亮实现**: 使用 Elasticsearch 的 Highlighting 功能，配合自定义标签 `<em class='highlight'>`。

### 2. 后端处理 (`SearchService`)
在 `searchProducts` 方法中，构建 NativeQuery 时配置高亮参数：
```java
HighlightFieldParameters parameters = HighlightFieldParameters.builder()
    .withPreTags(new String[]{"<em class='highlight'>"})
    .withPostTags(new String[]{"</em>"})
    .build();

Highlight highlight = new Highlight(Arrays.asList(
    new HighlightField("name", parameters),
    new HighlightField("description", parameters)
));
```
搜索结果返回后，将高亮片段映射到 `ProductDocument` 的 `highlightedName` 和 `highlightedDescription` 字段。

### 3. 前端展示
前端接收到带有 HTML 标签的高亮文本后，使用 `v-html` 指令进行渲染，并通过 CSS 样式突出显示：
```css
:deep(.highlight) {
  color: #e74c3c;
  font-weight: bold;
  background-color: rgba(231, 76, 60, 0.1);
}
```

## 未来扩展方向

1. **跨域推荐**: 结合用户社交关系和社区推荐
2. **序列推荐**: 基于Transformer的会话推荐
3. **知识图谱**: 融入商品知识图谱增强推荐
4. **强化学习**: 使用RL优化长期用户价值
5. **联邦学习**: 保护隐私的分布式模型训练

