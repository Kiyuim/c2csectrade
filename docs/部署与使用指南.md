# c2csectrade智能二手商品交易平台 - 部署与使用指南

## 系统概述

c2csecTrade 是一个完整的**分布式二手商品交易平台**，支持数码产品、服装鞋包、图书、家居生活、运动户外等多品类交易。系统已完成从单体应用到分布式架构的全面升级，集成了多种先进技术：

### 核心技术栈
- **后端框架**: Spring Boot 3.2.3
- **数据库**: MySQL 8.0 (主从复制支持)
- **缓存**: Redis 7 (分布式缓存 + 会话管理)
- **消息队列**: RabbitMQ 3.12 (事件驱动架构)
- **搜索引擎**: Elasticsearch 8.11
- **对象存储**: MinIO
- **前端框架**: Vue.js 3
- **容器化**: Docker + Docker Compose

### 高级推荐系统特性
✅ **协同过滤** (Item-Based CF)
✅ **基于内容推荐** (TF-IDF + 余弦相似度)
✅ **混合推荐** (Hybrid: CF + Content + Popularity)
✅ **神经网络协同过滤** (NCF: GMF + MLP)
✅ **实时推荐** (Real-time Stream Processing)
✅ **A/B测试框架** (Multi-variant Testing)
✅ **多目标优化** (CTR + CVR + Revenue + Diversity)

## 快速开始

### 1. 环境要求

- **Docker**: 20.10+
- **Docker Compose**: 2.0+
- **JDK**: 17+
- **Node.js**: 16+ (开发环境)
- **内存**: 最少 8GB (推荐 16GB)
- **磁盘**: 20GB+

### 2. 一键启动（生产环境）

**推荐方式 - 使用修复后的启动脚本：**
```bash
# 克隆项目
cd /Users/Kiyu/IdeaProjects/temaple/c2csectrade

# 使用完整启动脚本（推荐）⭐
./scripts/start-fixed.sh

# 或使用 Makefile
make up
```

**手动启动方式：**
```bash
# 启动所有服务
docker compose up -d

# 查看服务状态
docker compose ps

# 查看日志
docker compose logs -f backend
```

**启动脚本功能：**
- ✅ 自动检查并安装前端依赖（包括WebSocket库）
- ✅ 构建后端Maven项目
- ✅ 构建前端Vue.js应用
- ✅ 停止现有容器避免冲突
- ✅ 启动所有Docker服务
- ✅ 显示服务状态和访问地址

### 3. 服务访问地址

- **前端**: http://localhost:80
- **后端API**: http://localhost:8080
- **RabbitMQ管理界面**: http://localhost:15672 (admin/admin123)
- **MinIO控制台**: http://localhost:9001 (minioadmin/minioadmin)
- **Elasticsearch**: http://localhost:9200

### 4. 本地开发环境

```bash
# 启动依赖服务（MySQL, Redis, RabbitMQ等）
docker-compose up -d mysql redis rabbitmq elasticsearch minio

# 后端开发
cd /Users/Kiyu/IdeaProjects/temaple/c2csectrade
mvn clean install
mvn spring-boot:run

# 前端开发
cd frontend
npm install
npm run serve
```

## 推荐系统使用指南

### 1. 初始化推荐系统

系统首次启动时，会自动初始化推荐模型。也可以手动触发：

```bash
# 管理员账号登录后调用
curl -X POST http://localhost:8080/api/recommendations/compute \
  -H "Authorization: Bearer <admin-token>"
```

### 2. A/B测试配置

系统默认已配置5个推荐算法变体：

- **Control (20%)**: 协同过滤
- **Variant A (20%)**: 基于内容
- **Variant B (20%)**: 混合推荐
- **Variant C (20%)**: NCF神经网络
- **Variant D (20%)**: 实时推荐

用户会根据ID自动分配到不同变体组，系统会追踪各组的性能指标。

### 3. 查看A/B测试结果

```bash
# 获取实验结果
curl http://localhost:8080/api/recommendations/abtest/results/recommendation_algorithm_v1
```

响应示例：
```json
{
  "control": {
    "impressions": 10000,
    "clicks": 250,
    "conversions": 25,
    "ctr": 0.025,
    "cvr": 0.1,
    "totalRevenue": 2500.0
  },
  "variant_c": {
    "impressions": 10000,
    "clicks": 320,
    "conversions": 35,
    "ctr": 0.032,
    "cvr": 0.109,
    "totalRevenue": 3500.0
  }
}
```

### 4. 实时推荐追踪

用户的每次交互都会被实时处理：

```javascript
// 前端自动追踪
// 商品浏览
axios.post('/api/history/view', { productId: 123 });

// 加入收藏
axios.post('/api/favorites/add/123');

// 加入购物车
axios.post('/api/cart/add', { productId: 123, quantity: 1 });
```

后端自动：
- 更新用户兴趣画像
- 训练NCF模型
- 更新热度分数
- 发布到消息队列

## Redis使用说明

### 关键数据结构

```
# 用户浏览历史
history:user:{userId} -> Sorted Set (score=timestamp)

# 用户兴趣画像
profile:user:{userId} -> Hash (tag -> score)

# 协同过滤推荐
recommend:item-cf:{productId} -> Sorted Set (score=similarity)

# 内容相似度
recommend:content:{productId} -> Sorted Set (score=similarity)

# 商品热度
recommend:popularity -> Sorted Set (score=popularity)

# NCF模型参数
ncf:embedding:user:{userId} -> Array[32]
ncf:embedding:item:{productId} -> Array[32]

# 实时会话
realtime:session:{userId} -> List (最近20次交互)

# 实时趋势
realtime:trending -> Sorted Set (score=timestamp)

# A/B测试分组
abtest:user:{experimentId}:{userId} -> String (variant)

# 分布式锁
lock:{resource} -> String (lockId)
```

### Redis监控

```bash
# 查看Redis状态
docker exec c2csectrade-redis redis-cli INFO

# 查看内存使用
docker exec c2csectrade-redis redis-cli INFO memory

# 查看键数量
docker exec c2csectrade-redis redis-cli DBSIZE

# 查看慢查询
docker exec c2csectrade-redis redis-cli SLOWLOG GET 10
```

## RabbitMQ消息队列

### 交换机和队列

**订单交换机** (order.exchange):
- `order.paid.queue` - 订单支付成功
- `order.canceled.queue` - 订单取消
- `order.completed.queue` - 订单完成

**通知交换机** (notification.exchange):
- `notification.email.queue` - 邮件通知
- `notification.system.queue` - 系统消息

**推荐交换机** (recommendation.exchange):
- `recommendation.update.queue` - 推荐更新

**商品交换机** (product.exchange):
- `product.created.queue` - 商品创建
- `product.stock.low.queue` - 库存不足

### 消息示例

```java
// 发布订单支付事件
OrderPaidEvent event = new OrderPaidEvent(
    orderId, userId, amount, "alipay", new Date()
);
eventPublisher.publishOrderPaid(event);

// 消费者自动处理
@RabbitListener(queues = "order.paid.queue")
public void handleOrderPaid(OrderPaidEvent event) {
    // 发送通知
    // 更新推荐
    // 记录日志
}
```

### RabbitMQ管理

访问管理界面: http://localhost:15672

- 用户名: admin
- 密码: admin123

可以查看：
- 队列消息堆积
- 消费速率
- 消息吞吐量

## 性能优化建议

### 1. Redis优化

```properties
# 增加连接池
spring.data.redis.lettuce.pool.max-active=20
spring.data.redis.lettuce.pool.max-idle=10

# 启用Pipeline
# 批量操作时使用redisCacheService.batchSet()
```

### 2. RabbitMQ优化

```properties
# 增加预取数量
spring.rabbitmq.listener.simple.prefetch=10

# 启用消费者并发
spring.rabbitmq.listener.simple.concurrency=5
spring.rabbitmq.listener.simple.max-concurrency=10
```

### 3. 数据库优化

```sql
-- 添加索引
CREATE INDEX idx_product_category ON pms_product(category);
CREATE INDEX idx_product_status_stock ON pms_product(status, stock);
CREATE INDEX idx_product_created_at ON pms_product(created_at);

-- 定期清理过期数据
DELETE FROM browsing_history WHERE created_at < DATE_SUB(NOW(), INTERVAL 90 DAY);
```

### 4. 推荐系统优化

```bash
# 调整推荐计算频率（根据商品数量）
# 商品 < 1000: 每小时
# 商品 1000-10000: 每6小时
# 商品 > 10000: 每天

# 调整候选集大小
# 协同过滤: Top 50
# 内容相似度: Top 50
# 实时推荐: Top 30
```

## 监控和告警

### 1. 应用监控

```bash
# 查看健康状态
curl http://localhost:8080/actuator/health

# 查看指标
curl http://localhost:8080/actuator/metrics
```

### 2. 日志查看

```bash
# 应用日志
docker-compose logs -f backend

# Redis日志
docker-compose logs -f redis

# RabbitMQ日志
docker-compose logs -f rabbitmq
```

### 3. 性能指标

监控以下关键指标：
- **QPS**: 每秒查询数
- **响应时间**: P50, P95, P99
- **错误率**: 4xx, 5xx错误
- **缓存命中率**: > 95%
- **消息堆积**: < 1000

## 故障排查

### 问题1: 推荐结果为空

**原因**: 初始数据不足或计算未完成

**解决**:
```bash
# 检查Redis中是否有推荐数据
docker exec c2csectrade-redis redis-cli KEYS "recommend:*"

# 手动触发计算
curl -X POST http://localhost:8080/api/recommendations/compute \
  -H "Authorization: Bearer <admin-token>"
```

### 问题2: RabbitMQ消息堆积

**原因**: 消费者处理速度慢

**解决**:
```bash
# 增加消费者并发数
# 在application.properties中调整
spring.rabbitmq.listener.simple.concurrency=10

# 或临时清空队列（测试环境）
docker exec c2csectrade-rabbitmq rabbitmqctl purge_queue order.paid.queue
```

### 问题3: Redis内存溢出

**原因**: 缓存数据过多

**解决**:
```bash
# 查看内存使用
docker exec c2csectrade-redis redis-cli INFO memory

# 清理过期键
docker exec c2csectrade-redis redis-cli --scan --pattern "cache:*" | xargs redis-cli DEL

# 设置最大内存限制
docker exec c2csectrade-redis redis-cli CONFIG SET maxmemory 2gb
docker exec c2csectrade-redis redis-cli CONFIG SET maxmemory-policy allkeys-lru
```

## 扩展部署

### 水平扩展

```bash
# 扩展后端实例到3个
docker-compose up -d --scale backend=3

# 配置Nginx负载均衡
# 编辑nginx.conf
upstream backend {
    server backend1:8080;
    server backend2:8080;
    server backend3:8080;
}
```

### Redis集群

```yaml
# docker-compose.yml
redis-master:
  image: redis:7-alpine
  
redis-slave-1:
  image: redis:7-alpine
  command: redis-server --slaveof redis-master 6379
  
redis-slave-2:
  image: redis:7-alpine
  command: redis-server --slaveof redis-master 6379
```

## 安全建议

1. **修改默认密码**
   - MySQL: root密码
   - RabbitMQ: admin密码
   - Redis: 启用密码认证

2. **启用HTTPS**
   ```bash
   # 使用Let's Encrypt获取证书
   certbot certonly --standalone -d yourdomain.com
   ```

3. **限流配置**
   ```java
   // 使用Redis实现限流
   Long count = redisCacheService.incrementWithExpire(
       "ratelimit:user:" + userId, 60
   );
   if (count > 100) {
       throw new RateLimitException();
   }
   ```

4. **数据备份**
   ```bash
   # MySQL备份
   docker exec c2csectrade-mysql mysqldump -uroot -p trade > backup.sql
   
   # Redis备份
   docker exec c2csectrade-redis redis-cli BGSAVE
   ```

## 总结

系统已完成：
✅ 修复所有编译错误
✅ 深度集成Redis（缓存、锁、布隆过滤器）
✅ 引入RabbitMQ消息队列
✅ 实现神经网络协同过滤（NCF）
✅ 实现实时推荐系统
✅ 实现A/B测试框架
✅ 实现多目标优化
✅ 完整的分布式架构
✅ 生产级配置和监控

系统已经具备生产环境部署能力！🎉

